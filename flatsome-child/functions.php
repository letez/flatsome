...
function enqueueScripts($hook) {
    // CSS
    wp_enqueue_style( 'main-style', get_stylesheet_directory_uri() .'/style.css', array('flatsome-main-css'), '1.1', 'all');
...
function foot_js() {
    ...
    wp_enqueue_script('particles', 'https://cdn.jsdelivr.net/npm/particles.js@2.0.0/particles.min.js', array(), null, true);
...
add_shortcode( 'sitetitle', function() { return get_bloginfo('name'); } );
// SITE EMAIL SHORTCODE
add_shortcode( 'siteemail', function() { return get_bloginfo('admin_email'); } );
...
add_action('flatsome_footer', 'addSmallFooter');
